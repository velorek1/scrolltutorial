...
void scrollDemo(unsigned int X, unsigned int Y, char states[NUM_STATES][MAX_LENGTH], unsigned int displayLimit, unsigned int *currentIndex, unsigned int padding){
	unsigned int dummyI = 0;
	unsigned displayPage = (*currentIndex / displayLimit) * displayLimit; 
	size_t i=0;
        size_t k=0;
	char blank[5] = "    ";
...
int main() {
  ...
  char states[NUM_STATES][MAX_LENGTH] = {
   ...
   clrscr(); // Clear the screen with current background color
	...
	if (ch == -2){
	    if (currentIndex<NUM_STATES-1) currentIndex++;
	}
	if (ch == 10) {
	    gotoxy(1,18);
